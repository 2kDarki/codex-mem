...
Codex is the primary runtime described in this guide. Claude Code and Cursor still use the same worker and database as compatibility hosts.

### The Full Cycle

1. **Start Codex** - Context from recent sessions keeps accumulating automatically
2. **Work normally** - Every tool execution is captured
3. **Codex finishes responding** - The watcher records the completed turn and saves a summary
4. **Next session** - Previous work appears in context

### What Gets Captured

Every time Codex uses a tool, codex-mem captures it:
...
- And other supported Codex tool events
...
When a Codex turn finishes, a summary is automatically generated with:
...
When you start a new Codex session, the transcript watcher and context pipeline:
...
5. Inject formatted context into `~/.codex/AGENTS.md`
...
This means Codex "remembers" what happened in previous sessions.

## Manual Commands (Optional)

### Worker Management

The worker usually starts automatically once you initialize codex-mem. Manual commands below are optional.
...
# Stop worker service
codex-mem worker stop

# Restart worker service
codex-mem worker restart

# View worker logs
codex-mem worker logs
...
Context is stored in SQLite at `~/.Codex-mem/claude-mem.db`.

Query the database directly:

```bash
# Open database
sqlite3 ~/.Codex-mem/claude-mem.db
...
Codex-mem provides MCP tools for querying your project history. Simply ask naturally:
