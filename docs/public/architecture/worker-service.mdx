...
The worker service is a long-running HTTP API built with Express.js and managed natively by Bun. Codex-mem processes observations through the Claude Agent SDK separately from hook execution to prevent timeout issues.
