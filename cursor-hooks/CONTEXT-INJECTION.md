...
1. **Install**: `codex-mem cursor install` creates the initial context file
...
When you run `codex-mem cursor install`, the project is registered in `~/.Codex-mem/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated, even if it happens from Codex or another IDE working on the same project.
