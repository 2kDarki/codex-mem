...
async function runWelcome() {
  Wt2(import_picocolors.default.bgCyan(import_picocolors.default.black(" codex-mem installer ")));
  ...
  if (alreadyInstalled) {
    R2.warn("Existing codex-mem installation detected.");
  }
...
// src/utils/product-surface.ts
var DEFAULT_CODEX_MEM_DATA_DIR = "~/.Codex-mem";
var INSTALLER_IDE_OPTIONS = [
...
function buildNextStepsLines(settingsConfig, selectedIDEs) {
...
async function runIdeSelection() {
  ...
  if (selectedIDEs.includes("codex")) {
    R2.info("Codex: Transcript-based memory will be configured.");
  }
  if (selectedIDEs.includes("claude-code")) {
...
async function runInstallation(selectedIDEs) {
  ...
  if (selectedIDEs.includes("cursor")) {
    ...
    R2.info("Run: codex-mem cursor setup");
  }
  if (selectedIDEs.includes("codex")) {
    R2.info("Codex transcript configuration will be available after installation.");
    R2.info("Run: codex-mem codex init");
  }
}
...
function runCompletion(providerConfig, settingsConfig, selectedIDEs) {
  ...
  const nextStepsLines = buildNextStepsLines(settingsConfig, selectedIDEs).map((line) => {
    if (line.startsWith("View your memories: http://localhost:")) {
      return `View your memories: ${import_picocolors6.default.underline(`http://localhost:${settingsConfig.workerPort}`)}`;
    }
    if (line === "Run: codex-mem codex init") {
      return `Run: ${import_picocolors6.default.bold("codex-mem codex init")}`;
    }
    return line;
  });
  Vt2(nextStepsLines.join("\n"), "Next Steps");
  Gt(import_picocolors6.default.green("codex-mem installed successfully!"));
}
